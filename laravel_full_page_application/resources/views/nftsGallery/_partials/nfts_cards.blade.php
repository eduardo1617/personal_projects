@php
    ...
    $viewed = $nft->views->firstWhere('user_id', Auth::id());
...
<div class="card" id="nft_{{ $nft->id }}">
    ...
    <div class="card-footer">
        <form action="{{ $viewed
                        ? route('nfts.views.update', ['view' => $viewed, 'nft' => $nft->id])
                        : route('nfts.views.store', ['nft' => $nft->id])}}"
            method="post">
            @csrf
             @if($viewed)
                @method('PATCH')
...
{{--<a href="{{ route('nfts.show', $nft->id) }}" >   {{ route('views.store', [$nft->id]) }}--}}
